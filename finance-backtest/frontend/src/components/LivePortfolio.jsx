...
export default function LivePortfolio({ strat, spy }) {
  ...
  return (
    <section className="portfolio-overview flex flex-col gap-4">
      <Card 
        className={`portfolio-hero border-l-4 p-6 rounded-none ${isOnTrack ? 'border-green bg-[#10b981]/[0.02]' : 'border-amber bg-[#f59e0b]/[0.02]'}`}
      >
        <CardContent className="portfolio-hero-content p-0 flex justify-between items-center w-full">
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center">
              <span className={`text-[9px] font-black tracking-widest uppercase px-2 py-0.5 font-mono rounded-none ${isOnTrack ? 'bg-[#10b981]/10 text-green' : 'bg-[#f59e0b]/10 text-amber'}`}>
                {isOnTrack ? 'ON TRACK' : 'LAGGING BENCHMARK'}
              </span>
            </div>
            <h1 className="text-[18px] font-black text-text-strong m-0 tracking-tight leading-snug">
              {isOnTrack 
            ...
            <p className="text-[12px] text-text-2 leading-relaxed m-0 mt-1">
              {isOnTrack 
      ...
      {sectorData.length > 0 && (
        <Card className="p-5 bg-surface border border-border-2 rounded-none">
          ...
          <div className="h-6 w-full flex overflow-hidden mb-5 border border-border-2 rounded-none">

...
export const SuggestiveSearch: React.FC<SuggestiveSearchProps> = ({
  ...
  const filteredItems = useMemo(() => {
    ...
    return (searchItems || []).filter((item: any) => {
      const ticker = (item.ticker || item.name || item || "").toLowerCase();
      const sector = (item.sector || "").toLowerCase();
      return ticker.includes(q) || sector.includes(q);
    });
  }, [search, searchItems]);
  ...
  return (
    ...
    >
      ...
      {isFocused && search.trim() !== "" && (
        <div
          className="absolute left-0 right-0 top-full mt-2 z-50 bg-popover border border-border shadow-lg rounded-xl overflow-hidden max-h-60 overflow-y-auto"
          style={{ backdropFilter: "blur(12px)", backgroundColor: "var(--surface, rgba(23, 23, 23, 0.9))" }}
        >
          {filteredItems.length > 0 ? (
            filteredItems.map((item: any) => {
              const ticker = item.ticker || item.name || item;
              ...
              return (
                <div
                  ...
                  className="flex items-center justify-between px-4 py-2 hover:bg-muted cursor-pointer transition-colors text-sm"
                  style={{ borderBottom: "1px solid var(--border-2, rgba(255,255,255,0.05))" }}
                >
                  <div className="flex flex-col flex-1 text-left">
                    <span className="font-bold text-foreground" style={{ color: "var(--primary, #fafafa)" }}>{ticker}</span>
                    {sector && <span className="text-[10px] text-muted-foreground" style={{ color: "var(--muted-foreground, #a1a1a1)" }}>{sector}</span>}
                  </div>
                  <div 
                    className="text-[10px] font-mono text-muted-foreground uppercase border border-border px-1.5 py-0.5 rounded flex-shrink-0"
            ...
            <div className="p-4 text-center text-xs text-muted-foreground" style={{ color: "var(--muted-foreground, #a1a1a1)" }}>
